...
function injectRendererBundleFromPreload() {
  try {
    ...
    const channelBase = import_path2.default.resolve(__dirname, "..");
    const injectLogPath = import_path2.default.join(channelBase, "InAccord_inject.log");
    const preloadLoadedMarkerPath = import_path2.default.join(channelBase, "InAccord.preload_loaded.json");
    const runningMarkerPath = import_path2.default.join(channelBase, "InAccord.running.json");
    const injectStatusPath = import_path2.default.join(channelBase, "InAccord.inject_status.json");
...
`);
    ...
    logLine(`[preload] start pid=${process.pid} __dirname=${__dirname}`);
    writeInjectStatus("preload-start", { preloadDir: __dirname, channelBase });
    try {
      const payload = {
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        pid: process.pid,
    ...
    const bundlePath = import_path2.default.join(__dirname, "InAccord.js");
