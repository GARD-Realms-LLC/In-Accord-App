...
async function launchChannel(opts = {}) {
  ...
  try {
    // Packaged portable layout (electron-builder):
    //   __dirname === "...\\resources\\app.asar"
    //   real launcher script is bundled inside the asar at "app.asar\\scripts\\launcher.js"
    // Dev layout:
    //   __dirname === "...\\scripts\\launcher-ui"
    //   real launcher script lives at "...\\scripts\\launcher.js" (parent dir)

    const inAsar = path.join(__dirname, 'scripts', 'launcher.js');
    const devSibling = path.join(__dirname, '..', 'launcher.js');
    const pick = fs.existsSync(inAsar) ? inAsar : devSibling;

    const real = require(pick);
