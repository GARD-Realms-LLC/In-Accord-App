...
function usage() {
  console.log('Usage: node scripts/launcher.js --channel <stable|ptb|canary|development> [--patch-stable] [--preload <path>]');
  process.exit(1);
}
...
function getWinChannelName(c) {
  if (c === 'ptb') return 'DiscordPTB';
  if (c === 'canary') return 'DiscordCanary';
  if (c === 'development') return 'DiscordDevelopment';
  return 'Discord';
}

function getWinExeName(c) {
  if (c === 'ptb') return 'DiscordPTB.exe';
  if (c === 'canary') return 'DiscordCanary.exe';
  if (c === 'development') return 'DiscordDevelopment.exe';
  return 'Discord.exe';
}
...
if (args.length === 0) {
  ...
  else if (local && fs.existsSync(path.join(local, 'DiscordDevelopment'))) channel = 'development';
  else if (local && fs.existsSync(path.join(local, 'Discord'))) channel = 'stable';
  if (!channel) {
    console.error('No Discord installation detected. Provide --channel <stable|ptb|canary|development>');
    usage();
  }
  ...
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if ((a === '--channel' || a === '-c') && args[i+1]) {
      channel = normalizeChannel(args[++i]);
      if (!channel) usage();
    }
...
async function launchChannel(opts = {}) {
  ...
  const channelOpt = normalizeChannel(opts.channel || channel) || 'stable';
  ...
  if (platform === 'win32') {
    const local = process.env.LOCALAPPDATA || '';
    if (!local) throw new Error('LOCALAPPDATA is not set on Windows.');
    const channelName = getWinChannelName(channelOpt);
    ...
    const exeName = getWinExeName(channelOpt);
  ...
  if (platform === 'darwin') {
    ...
    const appName = channelOpt === 'stable'
      ? 'Discord'
      : channelOpt === 'ptb'
        ? 'DiscordPTB'
        : channelOpt === 'development'
          ? 'DiscordDevelopment'
          : 'DiscordCanary';
