...
(function main() {
    // Prefer resolving relative to this file, like BetterDiscord-style injectors.
    // This allows `discord_desktop_core/index.js` to require an absolute package path
    // (dist folder or InAccord.asar) without depending on a separate roaming install.
    ...
    try {
        ...
        const mainhookChannel = path.join(iaDir, `mainhook.${channelId}.js`);
        const mainhookGeneric = path.join(iaDir, "mainhook.js");
        const mainhook = process.env.INACCORD_MAINHOOK
            || (fs.existsSync(mainhookChannel) ? mainhookChannel : mainhookGeneric);
