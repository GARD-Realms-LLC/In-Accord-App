...
function resolveInAccordDir(): string {
    // When loaded via BetterDiscord-style injection, the runtime lives next to this file.
    // Keep the old roaming fallback for legacy installs.
    try {
...
(function main() {
    ...
    const rendererPreloadChannel = iaDir ? path.join(iaDir, `preload.${channelId}.js`) : "";
    const rendererPreloadGeneric = iaDir ? path.join(iaDir, "preload.js") : "";
    const rendererPreload = process.env.INACCORD_RENDERER_PRELOAD
        || (rendererPreloadChannel && fs.existsSync(rendererPreloadChannel) ? rendererPreloadChannel : rendererPreloadGeneric);

    const inAccordBundlePath = (() => {
        try {
            const dir = rendererPreload ? path.dirname(rendererPreload) : iaDir;
            if (!dir) return "";
            const ch = path.join(dir, `InAccord.${channelId}.js`);
            const gen = path.join(dir, "InAccord.js");
            return fs.existsSync(ch) ? ch : gen;
        }
        catch {}
        return "";
    })();
