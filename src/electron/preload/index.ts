...
function injectRendererBundleFromPreload() {
    try {
        ...
        // The build output is `dist/InAccord.js` (no channel suffix). Older installs used
        // channel-suffixed bundles. Prefer the channel file if present, otherwise fall
        // back to the generic bundle.
        const bundlePathChannel = path.join(__dirname, `InAccord.${channelId}.js`);
        const bundlePathGeneric = path.join(__dirname, "InAccord.js");
        const bundlePath = fs.existsSync(bundlePathChannel) ? bundlePathChannel : bundlePathGeneric;
        ...
        const inject = () => {
            ...
            try {
                ...
                const execInMainWorld = async () => {
                    ...
                    return await tryExec(preferred, wrapped);
                };
