...
export default class InAccord {
    ...
    static async injectRenderer(browserWindow: BrowserWindow) {
        ...
        if (!fs.existsSync(location)) return false; // TODO: cut a fatal log
        const content = fs.readFileSync(location).toString();

        const wrappedScript = `
            (() => {
                try {
                    const __iaCode = ${JSON.stringify(content)};
                    (0, eval)(__iaCode);
                    return true;
                } catch(error) {
                    console.error(error);
        ...
        if (!success) return false; // TODO: cut a fatal log
        return true;
    }
        ...
        browserWindow.webContents.on("dom-ready", () => {
            ...
            if (!hasCrashed) {
                ...
                const injectWithRetry = () => {
                    ...
                    this.injectRenderer(browserWindow).then((success) => {
                        if (success) return;
                        attempts++;
                        if (attempts >= maxAttempts) return;
                        setTimeout(injectWithRetry, 1200);
                    }).catch(() => {
                        attempts++;
                        if (attempts >= maxAttempts) return;
                        setTimeout(injectWithRetry, 1200);
                ...
                return setTimeout(injectWithRetry, 1000);
            }
