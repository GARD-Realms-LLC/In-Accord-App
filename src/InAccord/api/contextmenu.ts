...
let didInit = false;
let didInitPatcher = false;

// NOTE:
// This module used to access webpackRequire.m at import-time.
// When InAccord injects early (domReadyState=interactive), webpackRequire can still be null.
// That hard-crashed the entire injected bundle and prevented *everything* from loading.
...
let ContextMenuActions: {closeContextMenu: (...args: any[]) => any; openContextMenu: (...args: any[]) => any;} | null = null;

function canUseWebpack() {
    return !!(webpackRequire && (webpackRequire as any).m);
}

function ensureInitialized() {
    if (didInit && startupComplete) return true;
    if (!canUseWebpack()) {
        // Not ready yet; do not throw.
    ...
    try {
        ...
        if (!startupComplete) {
            ...
            let menuItemsId: string | undefined;
            let menuParser = "";

            const mods = (webpackRequire as any).m as Record<string, Function>;
            ...
            if (menuItemsId) {
                ...
                const contextMenuComponents = (webpackRequire as any)(menuItemsId);
        ...
        ContextMenuActions ??= {closeContextMenu: () => {}, openContextMenu: () => {}};
    }
...
class ContextMenu {
    ...
    open(event: MouseEvent, menuComponent: React.ComponentType<MenuRenderProps>, config?: MenuConfig) {
        ensureInitialized();
        return (ContextMenuActions ?? {openContextMenu: () => {}, closeContextMenu: () => {}}).openContextMenu(event, function (e) {
    ...
    close() {
        ensureInitialized();
        (ContextMenuActions ?? {closeContextMenu: () => {}}).closeContextMenu();
    }

    get Separator() {ensureInitialized(); return MenuComponents.Separator;}
    get CheckboxItem() {ensureInitialized(); return MenuComponents.CheckboxItem;}
    get RadioItem() {ensureInitialized(); return MenuComponents.RadioItem;}
    get ControlItem() {ensureInitialized(); return MenuComponents.ControlItem;}
    get Group() {ensureInitialized(); return MenuComponents.Group;}
    get Item() {ensureInitialized(); return MenuComponents.Item;}
    get Menu() {ensureInitialized(); return MenuComponents.Menu;}
}

Object.freeze(ContextMenu);
Object.freeze(ContextMenu.prototype);

// Patcher initialization is handled lazily by ensureInitialized() once webpack + components are ready.
...
// Kick off a safe, bounded init loop so context menu patching becomes available
// as soon as webpack attaches (without crashing when injected early).
try {
    let tries = 0;
