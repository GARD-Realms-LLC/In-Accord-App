import type {DriedAbortSignal, DriedResponse, NativeRequestInit, NativeRequestMethod} from "@common/native-fetch.ts";
...
async function fetch(input: string | URL | Request, init?: NativeRequestInit): Promise<Response> {
    ...
    try {
        // Only attempt to read body when present; GET/HEAD typically have no body.
        if (request.body) {
            const ab = await request.arrayBuffer();
            body = ab && ab.byteLength ? new Uint8Array(ab) : null;
        }
    }
    catch {
        body = null;
