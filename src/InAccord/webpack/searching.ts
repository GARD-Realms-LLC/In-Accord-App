...
export function getModule<T>(filter: Webpack.Filter, options: Webpack.Options = {}): T | undefined {
    ...
    // On newer Discord builds (especially Canary), webpack can take a moment to initialize.
    // Avoid throwing by touching `webpackRequire.c` before it exists.
    if (!webpackRequire || typeof (webpackRequire as any).c !== "object") {
