...
class CommandManager {
    ...
    static initialize() {
        try {
            ...
            // Command system patching is optional; do not take down all of In-Accord
            // (menus/toasts/modals/settings) if Discord internals changed.
            Logger.stacktrace("CommandManager", "Failed to initialize command system patches", error as Error);
        }
    }
    ...
    static #patchCommandSystem() {
        ...
        // Create our "local bot" author.
        // Discord frequently changes internal User models; sometimes the found export is not a constructor.
        // In those cases, a plain object is enough for message author display.
        const botSeed = {
            avatar: "inaccord",
        ...
        let botAuthor: any = null;
        try {
            const UserCtor: any = this.User;
            if (typeof UserCtor === "function") {
                try {
                    botAuthor = new UserCtor(botSeed);
                }
                catch {
                    // not constructable (e.g., factory function / module object)
                    botAuthor = null;
                }
            }
        }
