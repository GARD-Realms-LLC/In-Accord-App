    ...
    static onRemoved(node: Node, callback: () => void) {
        const observer = new MutationObserver((mutations) => {
            for (let m = 0; m < mutations.length; m++) {
                ...
                const parentMatch = nodes.some(parent => (parent as any).contains?.(node));
