...
const toPx = function (value: number | string) {
    return `${value}px`;
};

const styles = ["primary", "info", "success", "warn", "danger"] as const;
const sides = ["top", "right", "bottom", "left"] as const;

export type TooltipStyle = typeof styles[number];
        ...
        this.observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                ...
                const directMatch = nodes.indexOf(this.node) > -1;
                    const parentMatch = nodes.some(parent => (parent as any).contains?.(this.node));
