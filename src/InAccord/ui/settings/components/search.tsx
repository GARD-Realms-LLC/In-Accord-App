...
export default function Search({onChange, className, onKeyDown, placeholder}: SearchProps) {
    ...
    return <div className={"ia-search-wrapper" + (className ? ` ${className}` : "")}>
        <input onChange={change} onKeyDown={onKeyDown} type="text" className="ia-search" placeholder={placeholder} maxLength={50} value={value} ref={input} />
