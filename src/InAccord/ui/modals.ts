...
export default class Modals {
        ...
        if (Array.isArray(content) ? content.every(el => React.isValidElement(el)) : React.isValidElement(content)) {
            ...
            try {
                if (root && typeof root.render === "function") {
                    ...
                    anyDOM.render(content as ReactElement, contianer);
                }
                else {
    ...
    static makeStack() {
        ...
        if (root && typeof root.render === "function") {
            ...
            anyDOM.render(element as any, div);
        }
...
try {
    ...
    Logger.stacktrace("Modals", "Failed to initialize modal stack on module load", error as Error);
}