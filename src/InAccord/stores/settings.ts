...
export default new class SettingsManager extends Store {
    ...
    registerPanel(id: string, name: string, options: {onClick?: (o: any) => void; element?: ComponentType; order: number; type?: "addon" | "settings"; manager?: AddonManager; icon?: LucideIcon; discordIcon?: DiscordIcon; searchable?(): string[];}) {
        ...
        if (element) {
            // IMPORTANT:
            // Do not rely on DiscordModules.React.Component here.
            // Some Discord builds (or early startup timing) can make DiscordModules.React undefined,
            // which crashes startup and makes it look like In-Accord "does nothing".
            //
            // Panel renderers can be:
            // - a function/class component (callable)
            // - a pre-built React element/object
            if (typeof (element as any) === "function") {
