...
- **IPC Communication**: Use `src/InAccord/modules/ipc.ts` for mian↔renderer communication with event constants from `src/common/constants/ipcevents.ts`.
- **Addon System**: Plugins/themes managed via `PluginManager`/`ThemeManager` in `src/InAccord/modules/`. APIs exposed through `iaApi` (`src/InAccord/api/`).
...
- **IPC Events**: Register in mian process via `ipc.registerEvents()`, handle in renderer with `window.iaApi.IPC.on()`

## Examples
- **New Plugin API**: Add to `src/InAccord/api/index.ts`, export via iaApi object
