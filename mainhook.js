...
(function main() {
  ...
  const inAccordBundlePath = (() => {
    try {
      ...
      const gen = import_path.default.join(dir, "In-Accord.js");
  ...
  try {
    ensurePreloadOnSession(sessionMod.defaultSession, "immediate-defaultSession");
  } catch {
  }
  try {
    const all = typeof sessionMod.getAllSessions === "function" ? sessionMod.getAllSessions() || [] : [];
    for (const ses of all) ensurePreloadOnSession(ses, "immediate-getAllSessions");
  } catch {
  }
  const injectIntoWebContents = async (contents, reason) => {
    if (channelId !== "stable") return;
    try {
      ...
      try {
        if (contents.__inaccordInjected) return;
        contents.__inaccordInjected = true;
      } catch {
      }
